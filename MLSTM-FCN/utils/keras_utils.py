...
def train_model(model:Model, dataset_id, dataset_prefix, dataset_fold_id=None, epochs=50, batch_size=128, val_subset=None,
    ...
    # I guess all this cutoff stuff is in case you have too many variables
    # for a specific datapoint. You could choose to take first or last n.
    ...
    # basically just weighting each class according to its frequency
    ...
    # to_categorical transforms an index label into a vector
    # for example if there are three labels and given datapoint has label 0,
    # vector generated is [1,0,0]
    y_train = to_categorical(y_train, len(np.unique(y_train)))
    y_test = to_categorical(y_test, len(np.unique(y_test)))

    # is timeseries...? the factor is either cubic or sqaure root
    if is_timeseries:
        factor = 1. / np.cbrt(2)
    else:
        factor = 1. / np.sqrt(2)
    ...
    # saves a model to specified path, monitoring specific criteria. only overwrites if new best
