		...
		for split in ['train', 'dev']:
			...
			for emotion in ['anger', 'happiness', 'sadness', 'surprise']:
				...
				if keypoints in ['random', 'majority']:
					...
					emotion_scores_dir = os.path.join(MODELS_DIR, 'MultiChannelCNN', args.input, emotion, 'ind/pose/scores')

				files = [file for file in os.listdir(emotion_scores_dir) if file.endswith(".csv")]
				...
				for file in files:
					...
					if keypoints == file_keypoints or (keypoints+'-NO' == file_keypoints) \
						or (keypoints+'-lr' == file_keypoints[:-5]):
