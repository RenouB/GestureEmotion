	...
	for experiment, inputs in all_scores.items():
		...
		for input, files in inputs.items():
			...
			for file, scores in files.items():
				...
				best_acc = scores['av_scores']['dev']['acc'][best_epoch]
				...
				all_folds_best_epoch_acc = np.concatenate([np.expand_dims(scores['all']['dev'][k]['acc'][best_epoch], axis=1) for k in range(8)], axis=1)
				print(file)
				print(np.around(best_av * 100, decimals=2), np.around(best_acc * 100, decimals = 2))
				print(np.around(all_folds_best_epoch.std(axis=1) * 100, decimals=2), np.around(all_folds_best_epoch_acc.std(axis=1), decimals=2))
				print('\n')
